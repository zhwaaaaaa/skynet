    ...
    if (evt & Event_READABLE) {
        ...
        if (newFd != -1) {
            ...
            if (sa.ss_family == AF_INET) {
                ...
                char buf[64];
                // ip地址转化为字符串
                const char *ip = inet_ntop(AF_INET, (void *) &(s->sin_addr), buf, sizeof(buf));
                int port = ntohs(s->sin_port);
                channel = new ConnChannel(newFd, ip, port);

            } else {
                auto *s = (struct sockaddr_in6 *) &sa;
                char buf[64];
                const char *ip = inet_ntop(AF_INET, (void *) &(s->sin6_addr), buf, sizeof(buf));
